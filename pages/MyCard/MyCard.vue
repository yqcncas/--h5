<template>
	<view class="mycard" :style="{backgroundImage: bgFlag ? '' : 'url(../../static/image/index/5.png)'}">
		<view class="mycard-header">
			<view class="mycard-header-left" :class="{avtive: headerIndex == 0}" @click="handleTopIndex(0)">打卡记录</view>
			<view class="mycard-header-right" :class="{avtive: headerIndex == 1}" @click="handleTopIndex(1)">打卡奖励</view>
		</view>
		<scroll-view scroll-y="true" style="height: calc(100vh - 30rpx);" @scrolltolower="lower">
			<view class="mycard-main"  v-if="headerIndex == 0">
					<view class="mycard-main-item" v-for="(item, index) in myCardList" :key = "index">
						<view class="mycard-main-item-top">
							<view class="mycard-main-item-left">{{item.createTime}}</view>
							<view class="mycard-main-item-right">打卡</view>
						</view>
					</view>
			</view>
			
			<view class="mycard-jifen" v-if="headerIndex == 1">
				<view class="mycard-jifen-item" v-for="(item, index) in myCardList" :key = "index">
					...
					<view class="mycard-jifen-item-bottom">
						<view class="mycard-jifen-item-bottom-left">打卡</view>
...
<script>
	export default {
		...
		methods: {
			...
			async initMyCardList () {
				if(!this.hasFlag) return
				this.pageNum = ++this.pageNum
				let res = await this.$fetch(this.$api.remaindList,{pageNum: this.pageNum, pageSize: this.pageSize,signType: 1}, 'GET', 'FORM')
				console.log(res)
				if (res.total == 0) {
				...
				this.myCardList = [...this.myCardList, ...res.rows]
